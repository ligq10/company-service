 ...
 */
'use strict';
var caseAddControllers=angular.module('caseAddControllers',['caseAddServices']);

/**
 * 人员新增
 */
caseAddControllers.controller('caseAddCtrl',['$scope','$state','$timeout','$upload','loginSession','caseAddFactory',
   function($scope,$state,$timeout,$upload,loginSession,caseAddFactory){
	...
	$scope.caseAdd=function(){
		...
		if(undefined != loginUser
				...
				&& null != loginUser.uuid){
			$scope.companycase.createdBy = loginUser.uuid; 
			$scope.companycase.modifiedBy = loginUser.uuid; 

		}
	caseAddFactory.create($scope.companycase,function(response){
